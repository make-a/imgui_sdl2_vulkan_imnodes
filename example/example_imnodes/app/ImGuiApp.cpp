...
void ImGuiApp::Run()
{
    ...
    while (m_running)
    {
        ...
        if (currentWidth > 0 && currentHeight > 0 &&
            (m_swapChainRebuild || m_mainWindowData.Width != currentWidth ||
             m_mainWindowData.Height != currentHeight))
        {
...
void ImGuiApp::UploadFonts()
{
    ...
    VkCommandBuffer command_buffer =
        m_mainWindowData.Frames[m_mainWindowData.FrameIndex].CommandBuffer;
...
void ImGuiApp::InitVulkan()
{
    ...
    // --- 创建 Vulkan Instance (与原 SetupVulkan 类似) ---
    {
        ...
        if (!f_vkCreateDebugReportCallbackEXT)
        {
            fprintf(
                stderr,
                "Warning: vkCreateDebugReportCallbackEXT not found, debug reporting disabled.\n");
        }
        else
        {
            ...
            err = f_vkCreateDebugReportCallbackEXT(
                m_instance, &debug_report_ci, m_allocator, &m_debugReport);
            check_vk_result(err);
        }
#endif
    }
    ...
    // --- 选择物理设备 (GPU) ---
    m_physicalDevice =
        ImGui_ImplVulkanH_SelectPhysicalDevice(m_instance); // 使用 ImGui 提供的帮助函数
    ...
    // --- 创建逻辑设备 ---
    {
        ...
        vkEnumerateDeviceExtensionProperties(
            m_physicalDevice, nullptr, &properties_count, properties.Data);
    ...
    // --- 创建描述符池 ---
    {
        ...
        {
            throw AppInitError(
                "vkCreateDescriptorPool failed with error code: " + std::to_string(err));
        }
    }
}
...
#ifdef APP_USE_VULKAN_DEBUG_REPORT
VKAPI_ATTR VkBool32 VKAPI_CALL ImGuiApp::debug_report(
...
bool ImGuiApp::IsExtensionAvailable(
